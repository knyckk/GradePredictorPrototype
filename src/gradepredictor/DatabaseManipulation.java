...
public class DatabaseManipulation {
    ...
    public static void createPaper(Paper paper, Subject subject) {//method for creating a paper
        ...
        try ( Connection conn = DriverManager.getConnection(URL, "THope", DATABASEPASSWORD);  Statement statement = conn.createStatement()) { // creates the connection
            ...
            for (Question question : paper.getQuestions()) {//adds each question
                ...
                state.setBinaryStream(1, GradePredictor.imageToBinary(question.getQuestion()));
        ...
        try ( Connection conn = DriverManager.getConnection(URL, "THope", DATABASEPASSWORD);  Statement statement = conn.createStatement()) { // creates the connection
            ...
            if (willAdd) {//if class name doesnt exist
                while (newCode) {//until unique codes are created
                    i = 0;
                    newCode = false;
                    studentCode = GradePredictor.getCode();//creates codes
                    teacherCode = GradePredictor.getCode();
                ...
                statement.execute("INSERT INTO " + CLASSES//creates class
                        + " \n VALUES('" + name + "','" + studentCode + "','" + teacherCode + "', " + GradePredictor.getSubject().getID() + ");");
    ...
    public static Classroom classFromName(String className) {//method for getting a class from the class name
        Classroom toReturn = new Classroom(className, "", "", GradePredictor.getSubject());
        try ( Connection conn = DriverManager.getConnection(URL, "THope", DATABASEPASSWORD);  Statement statement = conn.createStatement()) { // creates the connection
            try ( ResultSet result = statement.executeQuery("SELECT * FROM " + CLASSES + " WHERE " + CLASSNAME + " = '" + className + "'")) {//selects class with a given classname
                while (result.next()) { //for every result
                    toReturn = new Classroom(className, result.getString(STUDENTCODE), result.getString(TEACHERCODE), GradePredictor.getSubject());//stores the class
                }
                conn.close();//closes  the connection
            } catch (SQLException e) {
                System.out.println("database error occured!" + e); //error message in select statements
    ...
    public static double getGradePerformance(Subject subject, Method method) {//method to return how well a revision method performs based on differences in a subject's grades
        ...
        try ( Connection conn = DriverManager.getConnection(URL, "THope", DATABASEPASSWORD);  Statement statement = conn.createStatement()) { // creates the connection
            try ( ResultSet result = statement.executeQuery("Select PredictedGrade,TargetGrade,FinalGrade\n"
                ...
                while (result.next()) { //for every result
                    toReturn += GradePredictor.ungrade((GradePredictor.gradeToInt(result.getString(FINALGRADE)) - GradePredictor.gradeToInt(result.getString(TARGETGRADE))));//calculates a percentage based on the difference between achieved grade and target grade and average grade boundaries
                    toReturn += GradePredictor.ungrade((GradePredictor.gradeToInt(result.getString(FINALGRADE)) - GradePredictor.gradeToInt(result.getString(PREDICTEDGRADE))));//calculates a percentage difference between achieved and predicted grade and average grade boundaries
                    count += 2;
                }
                conn.close();//closes  the connection
            } catch (SQLException e) {
    ...
    public static String getMostRecent(Subject subject, Student student) {//method to get most recent grade achieved by a student in a subject
        ...
        try ( Connection conn = DriverManager.getConnection(URL, "THope", DATABASEPASSWORD);  Statement statement = conn.createStatement()) { // creates the connection
            ...
            if (paperid != -1) {
                toReturn = GradePredictor.grade(score, getPaperBoundaries(paperid));//grades the paper and stores grade
            }
        } catch (SQLException e) {
            System.out.println("connection error occured!" + e); //error message in sql statement or connections
        }
