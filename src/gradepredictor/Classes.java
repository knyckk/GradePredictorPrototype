...
public class Classes extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        leaveBox.setModel(new javax.swing.DefaultComboBoxModel<>(DatabaseManipulation.getClasses(GradePredictor.getTeacher()).stream().filter(x -> x.getSubject().equals(GradePredictor.getSubject())).map(x -> x.getName()).toArray(String[]::new)));

        removeLbl.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        removeLbl.setText("Remove Class:");

        removeBox.setModel(new javax.swing.DefaultComboBoxModel<>(DatabaseManipulation.getClasses(GradePredictor.getTeacher()).stream().filter(x -> x.getSubject().equals(GradePredictor.getSubject())).map(x -> x.getName()).toArray(String[]::new)));

        currentBox.setModel(new javax.swing.DefaultComboBoxModel<>(DatabaseManipulation.getClasses(GradePredictor.getTeacher()).stream().filter(x -> x.getSubject().equals(GradePredictor.getSubject())).map(x -> x.getName()).toArray(String[]::new)));
    ...
    private void createClassBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_createClassBtnActionPerformed
        if (ValidationRoutines.presenceCheck(createClassFld.getText()) //checks if data has been entered
            ...
            if (DatabaseManipulation.createClass(createClassFld.getText(), GradePredictor.getTeacher())) {//creates the class            
    ...
    private void joinClassBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_joinClassBtnActionPerformed
        if (ValidationRoutines.presenceCheck(joinFld.getText())) { //checks that classcode has been entered
            String className = DatabaseManipulation.joinClass(joinFld.getText(), GradePredictor.getTeacher()); //joins the class if code was entered
            if(className != null) {
    ...
    private void leaveBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_leaveBtnActionPerformed
        if (leaveBox.getSelectedItem() != null) {
            DatabaseManipulation.leaveClass(leaveBox.getSelectedItem().toString(), GradePredictor.getTeacher()); //leaves the class
